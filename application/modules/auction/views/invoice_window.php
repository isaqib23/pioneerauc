...
.form-group label {  font-size: 14px; line-height: 22px; color: rgba(0, 0, 0, 0.9);}
        ...
        <div class="row justify-content-end">
          <div class="col-sm-6">
            <div class="form-group">
              ...
              <input type="text" disabled id="buyer_T_balance" class="form-control" name="" >
            </div>
          </div>
        </div>
      ...
      $.ajax({
        ...
        success : function(data) {
          ...
          if (objData.msg == 'success') { 
            ...
            $('#item_name').val(objData.data['item_row'][0]['name']);
            $('#make').val(objData.data['item_row'][0]['make_name']);
            $('#model').val(objData.data['item_row'][0]['model_name']);
            $('#registration_no').val(objData.data['item_row'][0]['registration_no']);
            $('#code').val(objData.data['item_row'][0]['seller_code']);
            $('#name').val(objData.data['item_row'][0]['fname']+' '+objData.data['item_row'][0]['lname']);
            $('#lot_no').val(objData.data['lot_no']['lot_no']);
            $('#seller_T_balance').val(objData.data['balance']['amount']);
  ...
  $('#buy_btn').on('click', function(event){
    ...
    if (seller_id == '' || buyer_id == '' || amount == '') {
      ...
      $.ajax({   
        ...
        data:{item_id:item_id, seller_id:seller_id, buyer_id:buyer_id, amount:amount, [token_name]:token_value},
        success: function(data) {
          objdata = $.parseJSON(data);
          if(objdata.msg == 'success')
          {
