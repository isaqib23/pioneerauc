...
class OnlineAuction extends MY_Controller {
    ...
    // live online auction detail page
    ...
    public function live_online_auction_view($auction_id)
    {
        ...
        if($last_bid->num_rows() > 0){
            ...
            $data['bid_status'] = $last_bid['bid_status'];
            $data['bid_type'] = $last_bid['bid_type'];
            $data['last_user_id'] = $last_bid['user_id'];
        }else{
                        ...
                        if (!empty($bid_amount) && $bid_amount['user_id'] != $user->id) {
                            if (!empty($bid_data)) {
                                ...
                                if (!$auto_bidder) {
                                  ...
                                  //  $this->sendPushNotificationTest("esZQprOETOqE3f4SqZDCQa:APA91bHVGKKH5PQSA2CAqC1KY0NgelSWkmknoSgIas5kr9WHqCC8JBCMJgLZCq-wzjG9B8yNNtPWZr-76ZbqstrTs-nTKmVCxszFl1ScnG8eqvyO2dLBSdNpWSpQlPkClPDCjfNyEEit");
                                }
                            }
                        }
...
/////  found extra code////////////////////////////////////////////////////
   ...
   public function get_lotting()
    ...
    public function mix_function(){
        ...
        $record['get_bid_log2'] = $this->get_bid_log2($echo,$auction_id,$item_id);
        $record['get_all_lots2'] = $this->get_all_lots2($auction_id);
        $record['get_winning_lots2']=  $this->get_winning_lots2($auction_id);
        $record['get_hall_auto_bids2'] = $this->get_hall_auto_bids2($auction_id);
        ...
        $record['get_winner_status_model2'] = $this->get_winner_status_model2($auction_id,$item_id);
    ...
    public function get_bid_log($echo=true,$auction_id=0,$item_id=0){
        $output = '';
        $language = $this->language;
        if($echo){
            $auction_id = $this->input->post('auctionId');
    ...
    public function get_bid_log2($echo=true,$auction_id=0,$item_id=0){
        $output = '';
        $language = $this->language;

        // $bid_amount = $this->input->post('bid_amount');

        //$this->db->order_by('id', 'DESC')->get_where('live_auction_bid_log', ['auction_id' => $auction_id])->result_array();
        ...
        if($echo){
            ...
            $output2 = ['error' => false, 'response' => $output];
            return $output2;
        }else{
            return $output;
        }
    ...
    public function get_winner_status_model()
    {
        $auction_id = $this->input->post('auctionId');
        $item_id = $this->input->post('itemId');
        ...
        $bidLog = $this->db->order_by('id', 'DESC')->get_where('live_auction_bid_log', ['auction_id' => $auction_id])->row_array();
            ...
            if ($bidLog['bid_status'] == 'win') {
                    $output = ['status' => true, 'uid' => $user_id, 'bid_amount' => $bid_amount];
                }else{
                    $output = ['status' => false];
                }
    ...
    public function get_winner_status_model2($auction_id,$item_id)
    {
    ...
    public function get_all_lots2($auction_id)
    {
    ...
    public function get_winning_lots2($auction_id)
    {
    ...
    public function get_hall_auto_bids2($auction_id)
    {

        if($auction_id){
            $user = $this->session->userdata('logged_in');
            if($user){
                $auto_bids = $this->db->get_where('bid_auto', ['auction_id' => $auction_id, 'user_id' => $user->id]);
                if($auto_bids->num_rows() > 0){
                    $auto_bids = $auto_bids->result_array();
                    $response = $this->load->view('auction/online_auction/hallAutoBids', ['auto_bids' => $auto_bids, 'language' => $this->language], true);
                    $output = ['status' => true, 'response' => $response];
...
*/
       ...
       $response = $this->load->view('auction/online_auction/itemFields',['fields' => $fields, 'category' => $data['category'], 'item' => $data['item'], 'language' => $this->language], true);
    ...
    public function get_item_fields_data2($auction_id,$item_id){
    ...
    // show open live bid items box data
    public function get_item_box_data(){
       $data = $this->input->post();
       $response='';
        if($data){
        $auction_id = $data['auctionId'];
        ...
        foreach ($datafields as $key => $fields)
        {
            $item_dynamic_fields_info = $this->oam->get_itemfields_byItemid($item_id,152);
            $fields['values'] = json_decode($fields['values'],true);  
            $fields['data-id'] = $fields['id'];
            if (!empty($fields['values'])) {
                foreach ($fields['values'] as $key => $options) {
       ...
       $response = $this->load->view('auction/online_auction/itemBoxData',['fields' => $fields, 'category' => $data['category'], 'item' => $data['item'], 'language' => $this->language], true);
    ...
    public function get_item_box_data2($auction_id,$item_id){
    ...
    public function get_current_lot2($auction_id,$item_id)
    {
    ...
    public function get_upcoming_auctions2($auction_id)
    {
            ...
            if($user){
                ...
                if ($totalBalanceRequiredToPlaceBid > $currentBidAmount) {
                    ...
                    if($result){
                        ...
                        // azadar work here
                        $current_bid = $this->db->order_by('id', 'DESC')->get_where('live_auction_bid_log', ['auction_id' =>$data['auction_id'], 'item_id' => $data['item_id']])->row_array();

                        $pusher_data['price'] = $bid_total_amount;
                        $pusher_data['bidder_id'] = $user->id;
                        $pusher_data['bid_type'] = $current_bid['bid_type'];

                        $log = $this->get_bid_log(false,$data['auction_id'],$data['item_id']);
                        $pusher_data['log'] = $log;

                        $pusher->trigger('ci_pusher', 'live-price', $pusher_data);
