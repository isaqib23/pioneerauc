...
class Common_Model extends CI_Model
{
	...
	public function filterSelect2($select2_array=array())
    {
        try{
            $page= $select2_array['page'];
            $resultCount = 3;
            $end = ($page - 1) * $resultCount;       
            $start = $end + $resultCount;
            if (!empty($select2_array['where'])) {
	            $this->db->where($select2_array['where']);
	            $row = $this->db->select($select2_array['columns'])->get($select2_array['table'])->result_array();
	            $count = count($row);
                $data = [];
                foreach ($row as $value) {
                	$title = json_decode($value[$select2_array['values']], true);
                	if (is_array($title)) {
                    	$data[] = ['id'=>$value['id'], 'value'=>$title['english'], 'total_count'=>$count];
                	} else {
                    	$data[] = ['id'=>$value['id'], 'value'=>$value[$select2_array['values']], 'total_count'=>$count];
                	}
                }
