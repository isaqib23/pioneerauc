      ...
      <?= $this->load->view('template/template_user_leftbar') ?>
        <div class="right-col">
            <div class="row align-items-center">
                <div class="col-sm-4">
                    <h1 class="section-title text-left"><?= $this->lang->line('favourite');?></h1>
                </div>
                <div class="col-sm-8">
                    <form class="customform">
                        <input type="hidden" name="<?= $this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />
                        <div class="wrap">
                            <input type="text" placeholder="<?= $this->lang->line('status');?>" class="form-control"  name="search_fav" id="search_fav">
                            <div class="icon">
                                <img src="<?= NEW_ASSETS_IMAGES; ?>search-icon.png">
                            </div>
                        </div>
                        <select class="selectpicker" title="<?= $this->lang->line('select_cat'); ?>" id="item_cat">
            ...
            <div class="deposit-wrapper">
                    <table id="fav_table" class="table_id display">
                        <thead>
                            <tr>
                      <th class="id"> <?= $this->lang->line('id');?></th>
                      <th data-orderable="false"></th>
                      <th class="name"><?= $this->lang->line('name');?></th>
                      <th class="detail"><?= $this->lang->line('detail');?></th>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        'dataColumnArray': [{'data':'id'},{'data':'image'},{'data':'name'},{'data':'detail'},{'data':'status'},{'data':'action'}]
        });
