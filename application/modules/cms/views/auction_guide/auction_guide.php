        ...
        <?php } ?>
         <?php 
            if(isset($row) && !empty($row)) {
                $title = json_decode($row['auction_guide_title']);
                $description = json_decode($row['auction_guide_description']);
                // $chairman_title = json_decode($row['chairman_title']);
            }
            ?>


            <form method="post" novalidate="" id="auction_guide" name="ozform" action="" enctype="multipart/form-data" class="form-horizontal form-label-left">
                ...
                <!-- <div class="item form-group">    
                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="policy_english_title">Auction Guide Image
                        <span class="required">*</span>
                    </label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="file"   id="image" name="image">
                        <?php ///if(isset($row)){
                            //$file = $this->db->get_where('files', ['id' => $row['auction_guide_image']])->row_array(); ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#send').on('click', function(event){
            ...
            if (validation == true) {
                var formData = new FormData($("#auction_guide")[0]);
                $.ajax({
                    ...
                    var objData = jQuery.parseJSON(data);
                       if (objData.error == false) { 
                        window.location = base_url + 'cms/auction_guide';
                        }else{
