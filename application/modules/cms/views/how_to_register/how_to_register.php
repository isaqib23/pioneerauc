        ...
        <?php } ?>
         <?php 
            if(isset($row) && !empty($row)) {
                $title = json_decode($row['register_title']);
                $description = json_decode($row['register_description']);
                // $chairman_title = json_decode($row['chairman_title']);
            }
            ?>


            <form method="post" novalidate="" id="how_to_register" name="ozform" action="" enctype="multipart/form-data" class="form-horizontal form-label-left">
                ...
                <!-- <div class="item form-group">    
                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="policy_english_title">Register Image
                        <span class="required">*</span>
                    </label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="file"   id="image" name="image">
                        <?php //if(isset($row)){
                            //$file = $this->db->get_where('files', ['id' => $row['register_image']])->row_array(); ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#send').on('click', function(event){
            ...
            if (validation == true) {
                var formData = new FormData($("#how_to_register")[0]);
                $.ajax({
                    ...
                    var objData = jQuery.parseJSON(data);
                       if (objData.error == false) { 
                        window.location = base_url + 'cms/how_to_register';
                        }else{
