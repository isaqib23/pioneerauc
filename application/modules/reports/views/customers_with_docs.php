...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Customers With Documents <small></small></h2>
      ...
      <div class="x_content">
        ...
        <form method="post" id="rptfrm" action="<?= base_url('reports/filter_customer_with_doc');?>">
          <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>">

              <div class="form-group col-md-3 col-sm-6 col-xs-6">
                <select name="docsType" class="form-control col-md-7 col-xs-12">
                  <?php if($docTypes){
                    foreach ($docTypes as $key => $docType) {
                      $title = json_decode($docType['name']);
                      ?>
                      <option value="<?= $docType['id']; ?>"><?= $title->english; ?></option>
                        <?php
                      }
                    } ?>
              ...
              <div class="form-group col-md-3 col-sm-6 col-xs-6">
                <select name="docsTypeFilter" class="form-control col-md-7 col-xs-12">
...
<script>
  ...
  $("#filter").on("click", function(event){
    ...
    var dtReports = dtBuilder({
      ...
      'orderColumn': 0,
      'order': 'ASC',
      'url' : "<?= base_url('reports/customers_with_docs_processing'); ?>",
      'ajaxData' : filters,
      'filterFields': [],
      'dbFields': [
        {'data':'id'},
