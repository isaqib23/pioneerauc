            ...
            <div class="control-group">
              <div class="controls">
                <div class="input-prepend input-group">
                  <select name="categoryId" id="categoryId" placeholder="Select Category" class="form-control col-md-12 col-xs-12">
                  <option value="all">All</option>
                    <?php foreach ($categories as $key => $category) { ?>
                      <option value="<?= $category['id'] ?>"><?= json_decode($category['title'])->english; ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
...
$("#filter").on("click", function(event){
  ...
  var dtReports = dtBuilder({
    ...
    'orderColumn': 0,
    'order': 'ASC',
    'url' : "<?= base_url('reports/item_stock_process'); ?>",
    'ajaxData' : filters,
    'filterFields': [],
    'dbFields': [
      {'data':'name'},
