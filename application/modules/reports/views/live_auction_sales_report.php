...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Live Auction Sales Report <small></small></h2>
            ...
            <select name="auctionId" class="form-control col-md-7 col-xs-12">
              ...
              if($liveAuctions){
                foreach ($liveAuctions as $key => $liveAuction) {
                  ...
                  ?>
                    <option value="<?= $liveAuction['id']; ?>"><?= $title->english; ?></option>
                  <?php
                }
              }
              ?>
...
var firstEmptySelect = true;
  ...
  $('#auctionId').select2({
      ajax: {
          url: '<?= base_url();?>reports/liveAuctionsSelect2',
          type: 'post',
          delay: '250',
          cache: false,
...
$("#filter").on("click", function(event){
  ...
  if(validation){
    ...
    $.ajax({
      url: "<?php echo base_url('reports/live_auction_sales_report_process'); ?>",
      type: "post",
      data: filters,
  ...
  /*var dtReports = dtBuilder({
    ...
    'filterFields': [],
    'dbFields': [
      {'data':'lot_no', 'sTitle':'Lot'},
      {'data':'make', 'sTitle':'Make'},
      {'data':'model', 'sTitle':'Model'},
      {'data':'item_name', 'sTitle':'Item'},
      {'data':'item_year', 'sTitle':'Year'},
      {'data':'registration_no', 'sTitle':'Reg No'},
      {'data':'auction_name', 'sTitle':'Auction'},
      {'data':'reserved_price', 'sTitle':'Reserved Price'},
      {'data':'seller_name', 'sTitle':'Seller Name'},
      {'data':'seller_code', 'sTitle':'Seller Code'},
      {'data':'buyer_name', 'sTitle':'Buyer Name'},
      {'data':'buyer_mobile', 'sTitle':'Buyer Mobile'},
      {'data':'buyer_code', 'sTitle':'Buyer Code'},
      {'data':'card_number', 'sTitle':'B/No'},
      {'data':'sold_price', 'sTitle':'Sales Price'},
      {'data':'item_status', 'sTitle':'Status'}
    ]
  });*/
});
</script>