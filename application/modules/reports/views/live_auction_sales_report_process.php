		...
		if($data){
			foreach ($data as $key => $value) {
				...
				<tr>
					...
					if($vat){
						$vat_value = 0;
						$total_charges_value_for_vat = array_sum($total_charges);
						if($vat['type'] == 'percentage'){
							$vat_value = ((float)$total_charges_value_for_vat * (float)$vat['value']) / 100;
						}else{
							$vat_value = $vat['value'];
						}
	...
	$(document).ready( function () {
		...
		$("#dt-sales-reports").DataTable({
	        ...
	        drawCallback: function () {
				...
				var totals = 15 + dynamicCols;
				//console.log(dynamicCols);

				for (var i = 1; i <= dynamicCols; i++) {
					$("#charges-"+i).html(api.column( (15+i), {page:'all'} ).data().sum());
				}
