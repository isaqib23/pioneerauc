...
class Users_Model extends CI_Model
{
	...
	public function get_user_by_crm_id($id)
	{
		$this->db->select('*');
		$this->db->from('users');
  ...
  //       return $result;
	 ...
	 public function get_all_user_with_role(){
    ...
    public function search_list($id)
    {
    	$this->db->select('*');
    	$this->db->from('users');
    	$this->db->wher('role',4);
    ...
    public function get_all_user_roles_list(){
        $this->db->select('*');
        $this->db->from('acl_roles');
        $this->db->order_by('id', 'ASC');
        $result = $this->db->get()->result_array();
    ...
    public function get_role_by_id($id){
	 ...
	 public function get_all_user_without_sales(){
        $this->db->select('users.*,acl_roles.name as role_name');
        $this->db->from('users');
        $this->db->join('acl_roles',' acl_roles.id = users.role');
        $this->db->where('users.status',1);
	 ...
	 public function get_all_user_sales_managers(){
        $this->db->select('users.*,acl_roles.name as role_name');
        $this->db->from('users');
        $this->db->join('acl_roles',' acl_roles.id = users.role');
        $this->db->where('users.status',1);
	...
	public function update_forgot_password($id,$data)
	{
