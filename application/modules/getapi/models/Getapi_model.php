...
class Getapi_model extends CI_Model {
    ...
    public function multiUpload2($field="", $config = array(), $sizes=array(),$user_id)
    {
        ...
        if( ($config) && (!empty($field)) ){
            ...
            for($i = 0; $i < $filesCount; $i++){
                ...
                if ( ! $this->upload->do_upload('file')){
                    ...
                    echo json_encode(array("status" => false, 'code' => $codee, "message" =>  $this->upload->display_errors(), 'appVersion' => $version));
                    exit();
                }else{
                    ...
                    if($result){
                        ...
                        $codee = http_response_code(200);
                        echo json_encode(array("status" => false, 'code' => $codee, "message" => 'Database Uploading Error', 'appVersion' => $version));
                        exit();
                    }
                }
            }
            ...
            $codee = http_response_code(200);
            echo json_encode(array("status" => false, 'code' => $codee, "message" => 'Invalid Data Passed to Upload', 'appVersion' => $version));
            exit();
        }
    }

    public function upload2($file="", $config = array(), $sizes = array(),$user_id)
    {
        $version = $this->config->item('appVersion');
    ...
    function get_login_user($id)
    {
        ...
        $this->db->join('files','files.id = users.picture', 'LEFT');
        $query = $this->db->get();
    ...
    function get_login_user2($id)
    {
        $this->db->select('users2.*,files.id as fileId,files.name as picture');
        $this->db->from('users2');
        $this->db->where('users2.id',$id);
        $this->db->join('files','files.id = users2.picture', 'LEFT');
        $query = $this->db->get();
    ...
    function check_email2_users2($email)
    {
        $this->db->select('*');
        $this->db->from('users2');
        $this->db->where('email', $email);
    ...
    public function login_check2($username, $password)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('email', $username);
    ...
    public function login_check3($username)
    {
        $this->db->select('*');
        $this->db->from('users');
    ...
    public function login_check4($username)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('email', $username);
    ...
    public function check_email_user_with_id($email)
    {
        $this->db->where('email',$email);
        $query = $this->db->get('users');
        if ($query->num_rows() > 0){
    ...
    public function check_email_user2($email,$id='')
    {
        $this->db->where('email',$email);
        if ($id) {
            $this->db->where('id !=',$id);
    ...
    public function check_email_user_google($email,$id='')
    {
        $this->db->where('email',$email);
        if ($id) {
            $this->db->where('id !=',$id);
    ...
    public function check_email_user_apple($email,$id='')
    {
        $this->db->where('email',$email);
        if ($id) {
            $this->db->where('id !=',$id);
    ...
    public function check_email_user_fb($email,$id='')
    {
        $this->db->where('email',$email);
        if ($id) {
            $this->db->where('id !=',$id);
    ...
    public function insert_user_details2($data)
    {
    ...
    public function update_user_details2($id,$data){
        $this->db->where('id', $id);
        $return=$this->db->update('users2', $data);
        if($return)
        {
            return $id;
        }
    ...
    public function verify_code2($code,$email)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('code',$code);
    ...
    public function verify_code2_user2($code,$email)
    {
        $this->db->select('*');
        $this->db->from('users2');
        $this->db->where('code',$code);
