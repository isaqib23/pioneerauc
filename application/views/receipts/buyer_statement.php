...
<body style="font-family: 'Open Sans', sans-serif; font-size: 13px; color: #000; margin: 0; padding-top: 30px;">
			...
			<tr style="width: 100%;">
				<td style="width: 100%; font-size: 14px; "  >
					...
					<p>
						...
						?>
						PO Box 666, <?php if(isset($contact['address'])){echo $address->english;}?> <br>
		...
		<table cellspacing="0" cellpadding="0" style="width: 100%; margin-top: 40px;">
			<tr style="width: 100%;">
				...
				<td style="width: 40%; font-size: 14px; "  >
					<h5 style="font-size: 18px; margin: 0; padding: 5px 10px; background: #000; color: #fff; text-align: center; margin: 0 0 15px 0" ><?= (isset($data['new_recipt']) && $data['new_recipt'] == 'yes') ? "" : "COPY"; ?> BUYER STATEMENT</h5>
			...
			<tr style="width: 100%;">
				<td style="width: 50%; font-size: 14px; border: 1px solid #000; padding: 50px 0 0 0; vertical-align: top;"  >
					<h4 style="margin: 0; font-size: 16px; text-align: center;">
						Brand / Model : <?php if(isset($item_info) && !empty($item_info)) {
						$item_info_name =json_decode($item_info['name']);
						echo $item_info_name->english; } ?>
					</h4>
					<table cellspacing="0" cellpadding="0" style="width: 100%; margin-top: 20px;">
						...
						if ($category['include_make_model'] == 'yes') { 
							...
							<tr>
								...
								<td style="padding: 5px 10px;" >
									<?php if(isset($make['title'])){echo $make_title->english;}?>
								</td>
							</tr>
							<tr>
								<td style="padding: 5px 10px;" >
								...
								<td style="padding: 5px 10px;" >
									<?php if(isset($model['title'])){echo $model_title->english;}?>
								</td>
							</tr>
							<tr>
								<td style="padding: 5px 10px;" >
	...
	<div class="container" style="max-width: 800px; margin: auto; padding: 0 15px 40px 15px; margin-top: 30px;">
		...
		<p style=" font-size: 14px; text-align: center; margin: 0" >
			THE ABOVE MENTIONED ITEM REMAINS THE PROPERTY OF THE SELLER UNTILL FULL PAYMENT HAS CLEARED.<br>
			(TITLE WILL THEN BE TRANSFERRED)
		</p>
		<h4 style="font-size: 16px; text-align: center; margin-top: 0;" >THE ABOVE MENTIONED ITEM IS SOLD WITHOUT GUARANTEE AND SOLD AS SEEN</h4>
		<div class="box" style="padding: 10px; border: 2px solid #000">
				...
				<tr>
					...
					<td>
						<h4 style="margin: 0; font-size: 17px;" >
							<span style="display: block; font-size: 14px; font-weight: 500;" >Item:-</span>
							<?php if(isset($item_info) && !empty($item_info)) {
								$item_name = json_decode($item_info['name']);
							 echo $item_name->english; } ?>
						</h4>
					</td>
					<td>
						<h4 style="margin: 0; font-size: 17px;" >
			...
			<?php
                ...
                $userName = '';
                if (isset($generated_by['b_statement'])) {
	                $generater_id = $generated_by['b_statement'];
